...
@section('content')
    ...
    <article class="container mt-28">
        <h1 class="text-xneutral-400 font-montserrat text-xl sm:text-2xl font-semibold">
            Welcome Address from the Rector of B-University
        </h1>
        <p class="text-xneutral-200 text-sm sm:text-base font-montserrat font-semibold">
            Achieving the Future with a Spirit of Unity
        </p>
        ...
        @else
            @foreach ($greetings as $greeting)
                <div class="grid grid-cols-1 sm:grid-cols-12 gap-8 mt-8">
                    <img src="{{ asset('storage/' . $greeting->image) }}" alt="Rector of B-University"
